...
{
    T value;
    int height, subtree_size, smaller_count;
    AVLNode *l, *r;
    AVLNode(T v) : value(v), height(1), subtree_size(1), smaller_count(0), l(nullptr), r(nullptr){};
};
...
public:
    ...
    {
        AVLNode<T> *n = new AVLNode(value);
        root = insert(root, n);
        return n;
    }

private:
    AVLNode<T> *insert(AVLNode<T> *p, AVLNode<T> *n)
    {
        if (!p)
            return n;
    ...
    int getHeight(AVLNode<T> *n) { return n ? n->height : 0; }

    int getSubtreeSize(AVLNode<T> *n) { return n ? n->subtree_size : 0; }

    int max(T a, T b) { return (a > b) ? a : b; }

    int getBalanceFactor(AVLNode<T> *n) { return n ? getHeight(n->l) - getHeight(n->r) : 0; }
    ...
    void updateSubtreeSize(AVLNode<T> *n)
    {
        if (n->r)
