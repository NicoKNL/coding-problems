...
struct GameState
{
    ...
    GameState(string top, string midtop, string midbot, string bottom)
    {
        hallway = "...........";
    ...
    bool isInEndState()
    {
        return room_a == "AAAA" && room_b == "BBBB" && room_c == "CCCC" && room_d == "DDDD";
    }
    ...
    void applyMove(Move m)
    {
        ...
        // cout << "dist: " << dist << " --> " << dist * COST[c] << endl;
        cost += (COST[c] * dist);
        // cout << "cost: " << cost << endl;
    ...
    bool isValidMove(Move m)
    {
        ...
        // cout << "LOC: " << m.from_loc << endl;
        // cout << endl;
        // cout << "TO: " << m.to << endl;
        // cout << "LOC: " << m.to_loc << endl;
        // cout << "----------------------" << endl;
        vector<bool> checks = {
...
private:
    ...
    vector<Move> getValidRoomMoves()
    {
        ...
        for (Location room : rooms)
        {
            ...
            if (room == ROOM_A && (r == "AAAA" || r == ".AAA" || r == "..AA" || r == "...A"))
                continue;
            if (room == ROOM_B && (r == "BBBB" || r == ".BBB" || r == "..BB" || r == "...B"))
                continue;
            if (room == ROOM_C && (r == "CCCC" || r == ".CCC" || r == "..CC" || r == "...C"))
                continue;
            if (room == ROOM_D && (r == "DDDD" || r == ".DDD" || r == "..DD" || r == "...D"))
                continue;
...
ostream &operator<<(ostream &os, const GameState &gs)
{
    ...
    os << "  |" << gs.room_a[2] << "|" << gs.room_b[2] << "|" << gs.room_c[2] << "|" << gs.room_d[2] << "|" << endl;
    os << "  |" << gs.room_a[3] << "|" << gs.room_b[3] << "|" << gs.room_c[3] << "|" << gs.room_d[3] << "|" << endl;
    os << "  |-------|" << endl;
    return os;
}
...
int dfs(GameState gs, Move move, int depth)
{
    ...
    if (gs.isInEndState())
    {
        ...
        if (MEMO.find(gs.hash()) == MEMO.end())
        {
            ...
            MEMO[gs.hash()] = {gs.cost, gs.cost};
            return MEMO[gs.hash()].second;
        }
        else
        {
            if (gs.cost < MEMO[gs.hash()].second)
            {
    ...
    {
        // cout << gs << endl;
        if (gs.cost >= MEMO[gs.hash()].first)
            return MEMO[gs.hash()].second;
        // cout
        //     << "memo: " << MEMO[gs.hash()].second << endl;
    }
    ...
    MEMO[gs.hash()] = {gs.cost, optimal_cost};
    return optimal_cost;
}
...
//     // cout << gs << endl;
//     gs.applyMove({ROOM_B, HALLWAY, 0, 5});
//     // cout << gs << endl;
//     gs.applyMove({ROOM_A, HALLWAY, 0, 3});
//     // cout << gs << endl;
//     gs.applyMove({HALLWAY, ROOM_B, 3, 0});
//     // cout << gs << endl;
//     gs.applyMove({HALLWAY, ROOM_A, 5, 0});
//     // cout << gs << endl;
//     // cout << gs.cost << endl;
// }
...
//     gs.room_b = ".B";
//     gs.hallway = ".....A.....";
//     Move m = {ROOM_A, HALLWAY, 0, 7};
//     cout << gs << endl;
//     cout << gs.isValidMove(m) << endl;
//     gs.applyMove(m);
//     cout << gs << endl;
// }
...
int main()
{
    ...
    string top, midtop, midbot, bottom;
    cin >> top >> midtop >> midbot >> bottom;

    GameState gs(top, midtop, midbot, bottom);
    cout << gs << endl;
    cout << INT32_MAX << endl;
    cout << dfs_start(gs) << endl;
}