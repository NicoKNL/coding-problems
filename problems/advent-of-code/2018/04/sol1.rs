...
fn main() {
    ...
    for (&guard_id, &time_asleep) in &guards {
        let time_asleep: u32 = time_asleep.iter().sum();
        if time_asleep > most_asleep_time {
            most_asleep_time = time_asleep;
            most_asleep_guard = guard_id;
        }
        most_asleep_minute = guards[&most_asleep_guard]
            .iter()
            .enumerate()
            .max_by_key(|&(_, value)| value)
            .map(|(index, _)| index)
            .expect("No minute found for most asleep guard");
    }
    println!(
        "Guard {} slept the most: {} minutes",
        most_asleep_guard, most_asleep_time
    );
